...
export default function DashboardPage() {
  ...
  const priorityAlerts = alerts.length ? alerts : MOCK_PRIORITY_ALERTS;
  const liquidityItems = liquidity.length ? liquidity : MOCK_LIQUIDITY;
  const liveAlerts = alerts.length ? alerts : MOCK_ALERTS;
  const recentConversions = conversions.length ? conversions : MOCK_CONVERSIONS;
  const recentActivity = activity.length ? activity : MOCK_ACTIVITY;
  const reviewQueue = kycQueue.length ? kycQueue : MOCK_KYC_QUEUE;
    ...
    <Box p={6} className="max-w-400 mx-auto w-full lg:p-8">
      <Stack gap={6}>
        ...
        <Grid cols={1} sm={2} lg={4} gap={6}>
          <StatCard
            label="24h Volume"
            value={(stats as any).volume_24h || (stats as any).total_volume_24h || "₦45.2M"}
            delta={12.5}
          ...
          <StatCard
            label="Active Users"
            value={(stats as any).active_users || (stats as any).users || "12,450"}
            delta={5.2}
          ...
          <StatCard
            ...
            value={(stats as any).pending_transactions || (stats as any).pending_txns || "142"}
            icon={<ArrowLeftRight size={20} />}
            status="warning"
          />
          <StatCard
            label="Success Rate"
            value={(stats as any).success_rate || "98.4%"}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <LiquiditySnapshot assets={liquidityItems as any} />
          </div>
          <div>
            <LiveAlerts alerts={liveAlerts as any} />
          </div>
        </div>

        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <RecentConversions conversions={recentConversions as any} />
          </div>
          <div>
            <RecentActivity entries={recentActivity as any} />
          </div>
        </div>

        <Grid cols={1} lg={2} gap={6}>
          <KYCReviewQueue items={reviewQueue as any} />
          <SystemHealthWidget items={systemHealth as any} />
        </Grid>
